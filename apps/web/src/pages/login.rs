...
use crate::components::{Alert, Button, Card, FormGroup, Input, Label};
use leptos::*;
...
pub fn LoginPage() -> impl IntoView {
    ...
    let handle_login = move || {
        ...
        let auth_async = auth_for_login.clone();
        let navigate_async = navigate_for_login.clone();
        spawn_local(async move {
            auth_async.login(&e, &p, &t).await;
            if auth_async.is_authenticated() {
                navigate_async("/", Default::default());
            }
        });
    };
        ...
        <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-slate-100 flex items-center justify-center px-4">
            <div class="w-full max-w-md">
                ...
                <Card class="shadow-xl">
                    ...
                    {move || auth.error.get().map(|e| view! { <Alert variant="error">{e}</Alert> })}
                    ...
                    <form class="space-y-5 mt-6">
                        ...
                        <FormGroup>
                            <Label for_id="tenant">"Tenant Slug"</Label>
                            <Input
                                input_type="text"
                                placeholder="test-madrasa"
                                value=tenant_slug.get()
